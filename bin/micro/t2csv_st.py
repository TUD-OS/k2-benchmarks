...
# generating plots, here concentrating on the evaluation of scheduler
# latencies
#
# attention: this script only works properly if request merging is disabled!
#
# expected arguments:
...
path  = sys.argv[1]
# list of background noise rates
...
# list should be sorted by integer suffix
...
for tdir in trace_dirs:
    ...
    for evt in tc.events:
        ...
        elif evt.name == "block_rq_issue2" and evt['bptr'] in outstanding:
...
csv = open(os.path.join(path, "data_schedtime.csv"), "w+")
csv.write(csvstring)
